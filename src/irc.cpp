...
// Copyright (c) 2011-2012 EroBitsCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("EroBitsCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #EroBitsCoinTEST0\r");
            Send(hSocket, "WHO #EroBitsCoinTEST0\r");
        } else {
            // randomly join #EroBitsCoin00-#EroBitsCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #EroBitsCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #EroBitsCoin%02d\r", channel_number).c_str());
        }
