...
uint256 hashGenesisBlock("0x901f4957a071c6873ff59b102b1486e18e98326a8e37b1660e99339d3908754d");
...
const string strMessageMagic = "EroBitsCoin Signed Message:\n";
...
{
    // EroBitsCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // EroBitsCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "erobits genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x9fc6d44ea954a8622eea54d9937f9cabb8407a6d6c31cf3ba959e43ca5cfa6c4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd7, 0xc2, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EroBitsCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EroBitsCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EroBitsCoinMiner(CWallet *pwallet)
{
    printf("EroBitsCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("EroBitsCoin-miner");
    ...
    try { loop {
        ...
        printf("Running EroBitsCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EroBitsCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EroBitsCoinMiner, pwallet));
}
